...
// ====
export async function getSecureUrl(id: string): Promise<{ url: string }> {
	const res = await axios.get(`/upload/url/${id}`);
	return res.data.data;
}
...
async function postDownloadAttachment(url: string): Promise<void> {
	// Get secure url
	const { url: secureUrl } = await getSecureUrl(url);
...
async function postUploadImagePrivately({ file, onProgressEvent }: UploadImageParams2): Promise<UploadImageResponse2> {
	const formData = new FormData();
	formData.append("file", file);
	...
	try {
		const res = await axios.post("/upload/private", formData, {
