...
export const getSender = (
	...
	const sender = recipients.find((r: RecipientResponseProps) => r._id !== loggedInUser);
	if (!noError) {
		if (!sender) {
...
export const getRecipient = (
	...
	const recipient = recipients.find((r: RecipientResponseProps) => r._id === loggedInUser);
	if (!noError) {
		if (!recipient) {
...
export const groupMessagesByDate = (msgs: CombinedMessageProps[]): Array<[string, CombinedMessageProps[]]> => {
	// Grouping the messages by date
	const grouped = msgs.reduce<Record<string, CombinedMessageProps[]>>((acc, message) => {
...
export const getLastMessage = (messages: CombinedMessageProps[]): string => {
	...
	const lstMsg = lastMessage?.content !== "" ? lastMessage?.content : (lastMessage?.attachments?.[0]?.type ?? "");
	return lstMsg ?? "";
};

export const getLastMessageTime = (messages: CombinedMessageProps[]): string => {
...
export const getConversationById = (
	...
	const convo = allConversations.find((c: CombinedConversationResponseProps) => c._id === messageId);
	if (!noError) {
		if (!convo) {
...
export const talentAndClientHasReviewed = (job: CollectionProps): boolean | undefined => {
	return (
...
export const getConversationHeader = (
	...
	return conversation.type === "DIRECT"
		? {
				_id: sender?._id,
				title: sender?.type === "creator" ? sender?.firstName : `${sender?.firstName}`,
...
export const removeSingleRecipientMessages = (
	...
	return allConversations.filter((message) => (message.recipients?.length ?? 0) > 1);
};
