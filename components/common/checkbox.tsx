...
>(({ className, ...props }, ref) => (
	<CheckboxPrimitive.Root
		ref={ref}
		className={cn(
			`focus-visible:ring-ring peer h-6 w-6 shrink-0 rounded-full border border-primary shadow focus-visible:outline-none
			focus-visible:ring-0 disabled:cursor-not-allowed disabled:opacity-50 data-[state=checked]:bg-primary
...
const CheckBox = ({ isChecked }: { isChecked: boolean }): JSX.Element => {
	return isChecked ? (
		<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
			<rect x="1" y="1" width="22" height="22" rx="5" fill="#3055B3" />
			<path
