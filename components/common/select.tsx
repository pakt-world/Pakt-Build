...
>(({ className, children, noChevron, ...props }, forwardedRef) => (
	<SelectPrimitive.Trigger
		className={cn(
			...
			placeholder:text-gray-300 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50`,
			className
		)}
		{...props}
		ref={forwardedRef}
...
export const SelectContent = React.forwardRef<SelectContentRef, SelectContentProps & { className?: string }>(
	({ className, children, ...props }, forwardedRef) => (
		<SelectPrimitive.Content
			...
			position="popper"
			className={cn(
				`relative z-50 min-w-[8rem] overflow-hidden rounded-md border bg-white data-[side=bottom]:translate-y-1
