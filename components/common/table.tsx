...
type ObjectKey = Record<string, any>;

interface TableProps<T extends ObjectKey> {
	data?: T[];
		...
		return (
			<div className="flex h-full min-h-[450px] w-full flex-col items-center justify-center gap-2">
				{[...Array(10)].map((_, index) => (
					<Skeleton className="flex h-[40px] w-full items-center justify-between gap-2 px-3 py-2" key={index}>
		...
		return (
			<div className="flex min-h-[450px] items-center justify-center">
				<div className="flex flex-col items-center gap-2">
					...
					<p className="text-center font-normal text-[#6C757D]">
						{emptyStateMessage ?? "Your Transactions Will Appear Here"}
					</p>
				</div>
			</div>
		);
			...
			<div className="mt-4 flow-root h-full">
				<div className="-mx-4 -my-4 overflow-x-auto sm:-mx-6 lg:-mx-8">
					<div className="inline-block min-w-full align-middle sm:px-6 lg:px-8">
						<div className="h-full max-h-[450px] min-w-full overflow-y-auto overflow-x-hidden border border-line shadow sm:rounded-lg">
							<table className="min-w-full table-fixed">
								...
								<tbody className="divide-y divide-line border-t border-line shadow">
