...
export const UserAvatar: FC<Props> = ({ image, score = 0, size = "md" }) => {
	return (
		<div
			className={`${getAvatarColor(score)} relative flex h-fit w-fit items-center justify-center rounded-full p-1`}
		>
			<div
				className={`absolute top-[0px] z-10 flex h-8 w-8 items-center justify-center rounded-full p-1 text-white ${getAvatarColor(score)}`}
				style={{
...
export const UserAvatar2: FC<Props> = ({ image, size = "md", useUpload = false, onUploadComplete }) => {
	...
	return (
		<div
			className="relative flex cursor-pointer items-center justify-center overflow-hidden rounded-full border bg-lime-50 text-white
				duration-200"
