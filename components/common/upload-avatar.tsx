	...
	return (
		<div className="flex h-[28px] w-full items-center justify-center rounded-lg border border-primary bg-green-200 px-2">
			<div className="h-[8px] w-full overflow-hidden rounded-full bg-green-100">
				<div
					className="h-full rounded-full bg-brand_gradient"
					style={{
						width: `${progress}%`,
					}}
				/>
...
export const UploadAvatar: FC<Props> = ({ image, size: previewImageSize = 180, onUploadComplete }) => {
	...
	return (
		<div className="flex flex-col items-center gap-2">
			<div
				{...getRootProps()}
				className="group relative flex cursor-pointer items-center justify-center overflow-hidden rounded-full bg-dark-bg duration-200
					sm:bg-[#ECFCE5]"
				style={{
			...
			>
				...
				<div className="absolute inset-0 flex items-center justify-center overflow-hidden rounded-full border-4 border-green-600">
					{previewImage ? (
						...
						<Image src="/images/no-user.png?v=2" alt="profile picture" layout="fill" objectFit="cover" />
					)}
				</div>

				<div
					className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-2 rounded-full p-2 text-center text-[#23C16B]
						opacity-0 duration-200 group-hover:bg-black group-hover:bg-opacity-70 group-hover:opacity-100"
				>
					<div>
						<GallerySvg size={24} />
					</div>
					<span className="flex flex-col gap-1 text-xs text-white">
						<span>
				...
				<div
					className="pointer-events-none absolute inset-0 z-30 flex flex-col items-center justify-center gap-1 rounded-full border
						border-red-200 bg-red-100 px-4 text-sm text-red-500 duration-200"
					style={{
			...
			{previewImage && !uploadImage.isLoading && !isFileTooLarge && (
				<button
					...
					className="h-[28px] w-full rounded-lg border border-primary bg-green-200 px-2 text-xs font-medium capitalize text-green-700
						duration-200 hover:bg-opacity-50"
				>
