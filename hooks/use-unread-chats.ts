...
export const getUnreadCount = (conversation: CombinedConversationResponseProps, loggedInUser: string): number => {
	let unreadCount = 0;
	const messages = conversation.messages ?? [];
	messages.forEach((message) => {
		// Check if the message is not read by the current user
		if (!message.readBy?.includes(loggedInUser) && message.user !== loggedInUser) {
			unreadCount++;
		}
	});
...
export const useUnreadChats = (conversations: CombinedConversationResponseProps[], loggedInUser: string): number => {
