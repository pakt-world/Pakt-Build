...
function useInfiniteScroll<T>({
	...
	const checkIfNeedToFetch = useCallback((): void => {
		...
		if (targetElement) {
			const targetBottomPosition = targetElement.getBoundingClientRect().bottom;
			const viewportHeight = window.innerHeight;

			// Fetch if the content doesn't fill the screen
			if (targetBottomPosition < viewportHeight && hasNextPage && !isFetchingNextPage) {
	...
	useEffect(() => {
		...
		const observer = new IntersectionObserver(
			(entries) => {
				if (entries[0]?.isIntersecting && !isFetchingNextPage) fetchMore();
			},
			{ threshold: 0.5 }
		);
