...
import type { UploadImageParams2, UploadImageResponse2 } from "@/lib/api/upload";
import { uploadAttachmentWithProgress } from "@/lib/api/upload";
import { MessageTypeEnums } from "@/lib/enums";
import { generateUniqueId } from "@/lib/utils";
import type { AttachmentsSendingProps, CombinedAttachmentType, TemporaryBubbleProps } from "@/providers/socket-types";
import Logger from "@/lib/utils/logger";
import { ACCEPTED_FILE_TYPES } from "./use-file-dropzone";
...
export const useMessageQueue = (
	...
	const validateRequiredProps = useCallback(() => {
	...
	const validateFileAttachment = useCallback((file: File) => {
		// Add specific checks for camera photos if needed
		...
		if (!isFileAccepted(file)) {
			throw new Error(`File ${file.name} is not an accepted type.`);
		}
			...
			setTemporaryMessages((prevMessages: TemporaryBubbleProps[]) =>
				prevMessages.map((msg: TemporaryBubbleProps) => {
					if (msg._id === messageId) {
						return {
							...msg,
							attachments: msg.attachments.map((att: CombinedAttachmentType) =>
								att.file?.name === fileName ? { ...att, uploadProgress: progress } : att
							),
						};
					}
					return msg;
	...
	const uploadAttachments = useCallback(
		async (messageId: string, attachments: AttachmentsSendingProps[]): Promise<CombinedAttachmentType[]> => {
			try {
				// Validate all files before uploading
				for (const attachment of attachments) {
					if (attachment.file) {
						validateFileAttachment(attachment.file);
					}
				}

				const uploadParams: UploadImageParams2[] = attachments.map((attachment) => ({
					file: attachment.file,
					onProgressEvent: (progressEvent: AxiosProgressEvent) => {
						const percentCompleted = Math.round((progressEvent.loaded * 100) / (progressEvent.total || 1));
						updateTemporaryMessageProgress(messageId, attachment.file.name, percentCompleted);
					},
				}));
				const uploadedResponses: UploadImageResponse2[] = await uploadAttachmentWithProgress(uploadParams);
				...
				const errorMessage = error instanceof Error ? error.message : "Error uploading attachments";
				Logger.error("Upload attachment error:", { error: errorMessage });
				toast.error(errorMessage);
				throw error;
			}
		},
		[updateTemporaryMessageProgress, validateFileAttachment]
	);
	...
	const sendMessage = useCallback(
		async (messageToSend: TemporaryBubbleProps) => {
			...
			try {
				...
				if (messageToSend.attachments && messageToSend.attachments.length > 0) {
					...
					uploadedAttachments = await uploadAttachments(messageToSend._id, validAttachments);
				}
				// Send the message
				await sendUserMessage(
					...
					() => {
						...
						setTemporaryMessages((prev) => prev.filter((msg) => msg._id !== messageToSend._id));
					}
				);
			} catch (error) {
				...
				setMessageQueue((prev) => prev.filter((msg) => msg._id !== messageToSend._id));
				setTemporaryMessages((prev) => prev.filter((msg) => msg._id !== messageToSend._id));
			} finally {
				setIsSending(false);
		...
		[sendUserMessage, setMessageQueue, setTemporaryMessages, uploadAttachments, validateRequiredProps]
	);
	...
	const sendQueuedMessages = useCallback(async () => {
		if (messageQueue.length > 0 && !isSending && !sendingPromise.current && !sendLock.current) {
	...
	const queueMessage = useCallback(
		(newMessage: TemporaryBubbleProps) => {
			// Logger.info("Queueing new message:", newMessage._id);
	...
	const handleSendMessage = useCallback(() => {
		...
		setText("");
		setImageFiles([]);
	}, [content, conversationId, imageFiles, queueMessage, recipientId, senderId, setImageFiles, setText]);
