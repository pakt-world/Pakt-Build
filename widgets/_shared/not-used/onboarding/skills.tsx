		...
		<button
			...
			className={clsx(
				"flex flex-col items-start gap-4 rounded-lg border p-4 text-title duration-200 hover:bg-gray-100 sm:min-w-[200px] sm:p-6",
				{
					...
					"border-[#3055B3] bg-[#3055B31A]": isActive,
				}
			)}
			type="button"
		>
...
export function Skills({ goToNextSlide }: SlideItemProps): React.JSX.Element {
	...
	useGetAccount({ enable: isClient ? true : false });
	const { user } = useUserState();
	const { profileCompleteness, firstName } = user ?? {};
	...
	return (
		<div className="z-[2] flex w-full shrink-0 flex-col items-center sm:gap-4">
			<div className="flex w-full flex-col gap-1 text-left">
				<p className="text-lg text-white sm:text-2xl sm:text-black">Great to meet you, {firstName}.</p>
				<span className="text-2xl font-bold text-white sm:text-4xl sm:text-[#1f2739]">
					What are you interested in?
				</span>
			</div>
			<div
				className="mt-4 grid w-full grid-cols-2 gap-4 rounded-2xl bg-white p-4 shadow sm:mt-9 sm:grid-cols-3 sm:gap-6 sm:rounded-none
			...
			>
				...
				<div className="col-span-2 mx-auto mt-2 w-full max-w-xs sm:hidden">
					<Button variant="primary" size="lg" fullWidth disabled={skill === ""} onClick={goToNextSlide}>
						Continue
					</Button>
				</div>
			</div>
			<div className="mx-auto mt-2 hidden w-full max-w-xs sm:block">
				<Button variant="primary" fullWidth disabled={skill === ""} onClick={goToNextSlide}>
					Continue
				</Button>
			</div>
		</div>
