...
export const JobSkills = ({ skills }: JobSkillsProps): JSX.Element => {
	...
	return (
		<div className="flex w-full flex-col border-b border-gray-200 bg-white px-5 py-4">
			...
			>
				<h3 className="text-base font-bold leading-normal tracking-wide text-black">Preferred Skills</h3>
				<ChevronRight
					className={`h-6 w-6 text-body transition-transform sm:hidden ${showSkill ? "rotate-90 transform" : ""}`}
				/>
			</Button>
			<div
				className={`flex flex-wrap gap-2 overflow-hidden transition-all ${showSkill ? "mt-2 h-fit" : "h-0"} sm:mt-0 sm:h-fit`}
			>
